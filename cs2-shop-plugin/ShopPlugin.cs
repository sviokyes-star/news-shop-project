...
public class ShopPlugin : BasePlugin
{
    ...
    [ConsoleCommand("css_a1", "Админ-панель: Показать спавны CT")]
    [RequiresPermissions("@css/root")]
    [CommandHelper(whoCanExecute: CommandUsage.CLIENT_ONLY)]
    public void OnA1Command(CCSPlayerController? player, CommandInfo command)
    {
        command.ArgString = "CT";
        OnShowSpawnsCommand(player, command);
    }

    [ConsoleCommand("css_a2", "Админ-панель: Показать спавны T")]
    [RequiresPermissions("@css/root")]
    [CommandHelper(whoCanExecute: CommandUsage.CLIENT_ONLY)]
    public void OnA2Command(CCSPlayerController? player, CommandInfo command)
    {
        command.ArgString = "T";
        OnShowSpawnsCommand(player, command);
    }

    [ConsoleCommand("css_a3", "Админ-панель: Скрыть маркеры")]
    [RequiresPermissions("@css/root")]
    [CommandHelper(whoCanExecute: CommandUsage.CLIENT_ONLY)]
    public void OnA3Command(CCSPlayerController? player, CommandInfo command)
    {
        OnHideSpawnsCommand(player, command);
    }

    [ConsoleCommand("css_a4", "Админ-панель: Добавить спавн CT")]
    [RequiresPermissions("@css/root")]
    [CommandHelper(whoCanExecute: CommandUsage.CLIENT_ONLY)]
    public void OnA4Command(CCSPlayerController? player, CommandInfo command)
    {
        command.ArgString = "CT";
        OnAddSpawnCommand(player, command);
    }

    [ConsoleCommand("css_a5", "Админ-панель: Добавить спавн T")]
    [RequiresPermissions("@css/root")]
    [CommandHelper(whoCanExecute: CommandUsage.CLIENT_ONLY)]
    public void OnA5Command(CCSPlayerController? player, CommandInfo command)
    {
        command.ArgString = "T";
        OnAddSpawnCommand(player, command);
    }

    [ConsoleCommand("css_a6", "Админ-панель: Удалить ближайший спавн")]
    [RequiresPermissions("@css/root")]
    [CommandHelper(whoCanExecute: CommandUsage.CLIENT_ONLY)]
    public void OnA6Command(CCSPlayerController? player, CommandInfo command)
    {
        OnRemoveSpawnCommand(player, command);
    }

    [ConsoleCommand("css_a7", "Админ-панель: Добавить подарок 1000")]
    [RequiresPermissions("@css/root")]
    [CommandHelper(whoCanExecute: CommandUsage.CLIENT_ONLY)]
    public void OnA7Command(CCSPlayerController? player, CommandInfo command)
    {
        command.ArgString = "1000";
        OnAddGiftCommand(player, command);
    }

    [ConsoleCommand("css_a8", "Админ-панель: Список подарков")]
    [RequiresPermissions("@css/root")]
    [CommandHelper(whoCanExecute: CommandUsage.CLIENT_AND_SERVER)]
    public void OnA8Command(CCSPlayerController? player, CommandInfo command)
    {
        OnListGiftsCommand(player, command);
    }

    [ConsoleCommand("css_a9", "Админ-панель: Удалить все подарки")]
    [RequiresPermissions("@css/root")]
    [CommandHelper(whoCanExecute: CommandUsage.CLIENT_AND_SERVER)]
    public void OnA9Command(CCSPlayerController? player, CommandInfo command)
    {
        OnRemoveGiftsCommand(player, command);
    }
