...
public class TimerPlugin : BasePlugin
{
    ...
    public void OnTimerCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        {
            player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Таймер не запущен");
            return;
        }
        ...
        if (timer.IsRunning)
        {
            ...
            player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Текущее время: {ChatColors.Yellow}{FormatTime(currentTime)}");
        }
        
        if (timer.BestTime != float.MaxValue)
        {
            player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Ваш лучший результат: {ChatColors.Gold}{FormatTime(timer.BestTime)}");
        }
        ...
        if (_mapRecords.ContainsKey(mapName))
        {
            ...
            player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Рекорд карты ({recordHolder}): {ChatColors.Purple}{FormatTime(_mapRecords[mapName])}");
        }
    }
    ...
    public void OnSetStartCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Зона старта установлена и сохранена!");
    ...
    public void OnSetEndCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Зона финиша установлена и сохранена!");
        player.PrintToChat($" {ChatColors.Yellow}Координаты: {position.X:F0}, {position.Y:F0}, {position.Z:F0}");
    }
    ...
    public void OnShowZonesCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (!_mapZones.ContainsKey(mapName))
        {
            player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Зоны не настроены");
            return;
        }
        ...
        {
            player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Зона старта:");
        ...
        {
            player.PrintToChat($" {ChatColors.Gold}Okyes | Зона старта не установлена");
        }

        if (zones.EndMin != null && zones.EndMax != null)
        {
            player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Зона финиша:");
        ...
        {
            player.PrintToChat($" {ChatColors.Gold}Okyes | Зона финиша не установлена");
        }
    }
    ...
    public void OnSetRecordHolderCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        {
            if (player != null)
                player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} На этой карте нет рекорда");
            return;
        }
        ...
        if (player != null)
            player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Рекордсмен установлен: {holderName}");
    ...
    public void OnTopCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        {
            player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Пока нет результатов");
            return;
        }

        player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Топ-10 результатов:");
    ...
    private void StopTimer(CCSPlayerController player)
    {
        ...
        if (isNewRecord)
        {
            ...
            player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Новый личный рекорд: {ChatColors.Gold}{FormatTime(finalTime)}!");
        }
        else
        {
            player.PrintToChat($" {ChatColors.Gold}Okyes |{ChatColors.Default} Время прохождения: {ChatColors.Yellow}{FormatTime(finalTime)}");
        }
        ...
        if (!_mapRecords.ContainsKey(mapName) || finalTime < _mapRecords[mapName])
        {
            ...
            Server.PrintToChatAll($" {ChatColors.Gold}Okyes |{ChatColors.Default} {player.PlayerName} установил новый рекорд карты: {ChatColors.Purple}{FormatTime(finalTime)}!");
        }
    }
